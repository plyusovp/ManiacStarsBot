...
async def add_user(user_id, username, full_name, referrer_id=None):
    async with connect() as db:
        ...
        if await cursor.fetchone() is None:
            await db.execute(
                "INSERT INTO users (user_id, username, full_name, invited_by, registration_date, last_seen) VALUES (?, ?, ?, ?, ?, ?)",
                (
                    user_id,
                    username,
                    full_name,
...
async def update_duel_stats(db, winner_id: int, loser_id: int):
    ...
    await db.execute(
        "UPDATE users SET duel_losses = duel_losses + 1 WHERE user_id = ?", (loser_id,)
    )
...
async def finish_duel_atomic(match_id: int, winner_id: int, loser_id: int, prize: int):
    async with connect() as db:
        try:
            ...
            await db.execute(
                "UPDATE duel_matches SET state = 'finished' WHERE id = ?", (match_id,)
            )
            await db.execute("COMMIT;")
        except Exception as e:
            await db.execute("ROLLBACK;")
...
async def finish_timer_match_atomic(
    match_id: int,
    winner_id: int = None,
    prize: int = 0,
    is_draw: bool = False,
