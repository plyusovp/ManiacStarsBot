...
async def init_db():
    ...
    async with connect() as db:
        ...
        if "risk_level" not in columns:
            await db.execute("ALTER TABLE users ADD COLUMN risk_level INTEGER NOT NULL DEFAULT 0")
...
async def check_idempotency_key(db: aiosqlite.Connection, key: str, user_id: int) -> bool:
    """
...
async def spend_balance(
    user_id: int, amount: int, reason: str, ref_id: Optional[str] = None, idem_key: Optional[str] = None
) -> bool:
    ...
    async with connect() as db:
        try:
            ...
            if not await check_idempotency_key(db, idem_key, user_id):
                await db.commit()
                cursor = await db.execute("SELECT id FROM rewards WHERE user_id = ? ORDER BY id DESC LIMIT 1", (user_id,))
                last_reward = await cursor.fetchone()
                return {"success": True, "already_exists": True, "reward_id": last_reward['id'] if last_reward else None}
            ...
            hold_success = await _change_balance(db, user_id, -stars_cost, "reward_hold")
            if not hold_success:
                await db.rollback()
...
async def _update_reward_status(
    db: aiosqlite.Connection, reward_id: int, new_status: str, admin_id: int, notes: Optional[str] = None
):
...
async def reject_reward(reward_id: int, admin_id: int, notes: str) -> bool:
    ...
    async with connect() as db:
        try:
            await db.execute("BEGIN IMMEDIATE;")
            cursor = await db.execute(
                "SELECT user_id, stars_cost, status FROM rewards WHERE id = ?", (reward_id,)
            )
            ...
            await _change_balance(db, reward['user_id'], reward['stars_cost'], "reward_revert", f"reward:{reward_id}")
...
async def approve_reward(reward_id: int, admin_id: int, notes: Optional[str] = None) -> bool:
    """Одобряет заявку."""
    async with connect() as db:
        try:
            await db.execute("BEGIN IMMEDIATE;")
...
async def fulfill_reward(reward_id: int, admin_id: int, notes: Optional[str] = None) -> bool:
...
async def get_pending_rewards_count() -> int:
    ...
    async with connect() as db:
        cursor = await db.execute("SELECT COUNT(id) as count FROM rewards WHERE status = 'pending'")
        ...
        if await cursor.fetchone() is None:
            ...
            if referrer_id:
                ...
                await add_balance_with_checks(
                    referrer_id, 5, "referral_bonus", str(user_id)
                )
    ...
    async with connect() as db:
        ...
        if time_passed > 86400:
            ...
            try:
                ...
                await db.rollback()
                print(f"Error in get_daily_bonus transaction: {e}")
                return {"status": "error"}
        else:
...
async def grant_achievement(user_id, ach_id, bot: Bot) -> bool:
    ...
    async with connect() as db:
        try:
            ...
            await db.rollback()
            print(f"Error in grant_achievement transaction: {e}")
            return False
    ...
    async with connect() as db:
        try:
            ...
            p1_success = await _change_balance(
                db, p1_id, -stake, "duel_stake_hold"
            )
            if not p1_success:
                await db.rollback()
                return None
            ...
            p2_success = await _change_balance(
                db, p2_id, -stake, "duel_stake_hold"
            )
            if not p2_success:
    ...
    async with connect() as db:
        try:
            ...
            if recent_games and recent_games[0] > 5:
                logger.warning(f"ANTI-ABUSE: Users {winner_id} and {loser_id} played {recent_games[0]} games recently. Match ID: {match_id}")
...
async def force_surrender_duel(match_id: int, user_id: int):
    ...
    async with connect() as db:
        cursor = await db.execute("SELECT state, stake FROM duel_matches WHERE id = ?", (match_id,))
        match_info = await cursor.fetchone()
        if not match_info or match_info['state'] != 'active':
            return

        cursor = await db.execute("SELECT user_id FROM duel_players WHERE match_id = ? AND user_id != ?", (match_id, user_id))
        opponent = await cursor.fetchone()
        if not opponent:
            await add_balance_unrestricted(user_id, match_info['stake'], "duel_stuck_refund", f"stuck:{match_id}")
        ...
        await add_balance_unrestricted(winner_id, match_info['stake'], "duel_stuck_refund", f"stuck:{match_id}")
...
async def create_timer_match(p1_id: int, p2_id: int, stake: int, bank: int):
    """Создает таймер-матч, ставки уже списаны."""
    async with connect() as db:
...
async def finish_timer_match(
    ...
    async with connect() as db:
        try:
            await db.execute("BEGIN IMMEDIATE;")
            match_details = await db.execute(
                "SELECT stake, p1.user_id, p2.user_id FROM timer_matches tm "
                "JOIN timer_players p1 ON tm.id = p1.match_id AND p1.role = 'p1' "
                "JOIN timer_players p2 ON tm.id = p2.match_id AND p2.role = 'p2' "
                "WHERE tm.id = ?",
                (match_id,),
            )
            details = await match_details.fetchone()
            if not details:
                raise ValueError("Match details not found")
            stake, p1_id, p2_id = details
            ...
            if is_draw:
                ...
                await db.execute(
                    "UPDATE timer_matches SET state = 'draw', bank = ? WHERE id = ?",
                    (new_bank, match_id),
                )
            elif winner_id:
                prize = (stake * 2) - int((stake * 2) * 0.07)
                await add_balance_with_checks(
                    winner_id, prize + stake, "timer_win", str(match_id)
                )
                await db.execute(
