...
def duel_stuck_keyboard():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🏳️ Сдаться и выйти (поражение)", callback_data="duel_leave_active")],
...
def duel_round_keyboard(hand: list, match_id: int, boosts_left: int, replace_left: int):
    ...
    for card in sorted(hand, key=lambda x: x[0] if isinstance(x, tuple) else x):
        ...
        card_buttons.append(InlineKeyboardButton(text=card_text, callback_data=f"duel_play:{match_id}:{card_value}:{original_value}"))
    builder.row(*card_buttons)
    
    action_buttons = []
    if boosts_left > 0:
        action_buttons.append(InlineKeyboardButton(text=f"⚡ Усилить ({boosts_left})", callback_data=f"duel_boost:{match_id}"))
    if replace_left > 0:
        action_buttons.append(InlineKeyboardButton(text=f"🔄 Заменить ({replace_left})", callback_data=f"duel_replace:{match_id}"))
    
    if action_buttons:
        builder.row(*action_buttons)
    
    builder.row(InlineKeyboardButton(text="🏳️ Сдаться", callback_data=f"duel_surrender:{match_id}"))
...
def duel_boost_choice_keyboard(hand: list, match_id: int):
    ...
    for card in sorted(hand, key=lambda x: x[0] if isinstance(x, tuple) else x):
        ...
        buttons.append(InlineKeyboardButton(text=f"⚡ {card_value}", callback_data=f"duel_boost_choice:{match_id}:{card_value}"))
    builder.row(*buttons)
    builder.row(InlineKeyboardButton(text="🔙 Отмена", callback_data=f"duel_cancel_action:{match_id}"))
    return builder.as_markup()
...
def duel_replace_choice_keyboard(hand: list, match_id: int):
    ...
    for card in sorted(hand, key=lambda x: x[0] if isinstance(x, tuple) else x):
        card_value = card[0] if isinstance(card, tuple) else card
        buttons.append(InlineKeyboardButton(text=f"🔄 {card_value}", callback_data=f"duel_replace_choice:{match_id}:{card_value}"))
    builder.row(*buttons)
    builder.row(InlineKeyboardButton(text="🔙 Отмена", callback_data=f"duel_cancel_action:{match_id}"))
    return builder.as_markup()
...
def duel_surrender_confirm_keyboard(match_id: int):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Да, сдаться", callback_data=f"duel_surrender_confirm:{match_id}"),
            InlineKeyboardButton(text="❌ Нет, в бой!", callback_data=f"duel_cancel_action:{match_id}")
        ]
    ])
...
def coinflip_choice_keyboard():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="🦅 Орёл", callback_data="coinflip_play:heads"),
            InlineKeyboardButton(text="🪙 Решка", callback_data="coinflip_play:tails")
        ],
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="game_coinflip_cancel")]
    ])

def coinflip_continue_keyboard(prize: int):
    """Клавиатура для выбора: забрать выигрыш или рискнуть."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Забрать {prize} ⭐", callback_data="coinflip_cashout")],
        [InlineKeyboardButton(text="🔥 Рискнуть дальше!", callback_data="coinflip_continue")]
    ])
...
def back_to_duels_keyboard():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Назад к выбору ставки", callback_data="game_duel")]
    ])
...
def withdraw_menu():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="25 ⭐ (🎁)", callback_data="gift_25_present")
        ],
        [
            InlineKeyboardButton(text="50 ⭐ (🍾)", callback_data="gift_50_champagne"),
            InlineKeyboardButton(text="50 ⭐ (💐)", callback_data="gift_50_flowers")
        ],
        [
        ...
        [InlineKeyboardButton(text="100 ⭐ (💎)", callback_data="gift_100_diamond")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_main")]
    ])
...
def achievements_menu_keyboard(all_achievements: list, user_achievements: list, page: int = 1):
    ...
    for ach_id, ach_name in all_achievements[start_index:end_index]:
        ...
        builder.row(InlineKeyboardButton(text=f"{status_icon} {ach_name}", callback_data="ignore_click"))

    nav_buttons = []
    if page > 1:
        nav_buttons.append(InlineKeyboardButton(text="« Пред.", callback_data=f"achievements_page:{page-1}"))
    if end_index < len(all_achievements):
        nav_buttons.append(InlineKeyboardButton(text="След. »", callback_data=f"achievements_page:{page+1}"))
