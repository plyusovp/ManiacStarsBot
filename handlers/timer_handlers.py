...
from keyboards.inline import (timer_finish_keyboard, timer_game_keyboard,
                              timer_rematch_offer_keyboard,
                              timer_stake_keyboard, timer_stuck_keyboard)
...
async def resolve_timer_match(bot: Bot, match: TimerMatch):
    ...
    await asyncio.gather(
        edit_caption_safe(
            ...
            match.p1_message_id,
            result_text,
            timer_finish_keyboard(match.match_id, p1_opponent_id),
        ),
        edit_caption_safe(
            bot,
            match.p2_id,
            match.p2_message_id,
            result_text,
            timer_finish_keyboard(match.match_id, p2_opponent_id),
        ),
    )
        ...
        try:
            await asyncio.gather(
                edit_caption_safe(
                    ...
                    match.p1_message_id,
                    text,
                    timer_game_keyboard(match.match_id),
                ),
                edit_caption_safe(
                    bot,
                    match.p2_id,
...
async def timer_menu_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if await db.get_active_timer_id(user_id):
        ...
        return await edit_caption_safe(
            callback.bot,
...
async def find_timer_handler(callback: CallbackQuery, bot: Bot):
    ...
    if opponent_data and opponent_data["user_id"] != user_id:
        ...
        await edit_caption_safe(
            ...
            callback.message.chat.id,
            callback.message.message_id,
            "✅ Соперник найден! Запуск...",
        )
        await edit_caption_safe(
            bot, opponent_id, opponent_msg_id, "✅ Соперник найден! Запуск..."
        )
        await asyncio.sleep(1)
        await start_timer_match(
            bot,
            opponent_id,
            user_id,
            stake,
        ...
        await edit_caption_safe(
            ...
            callback.message.chat.id,
            callback.message.message_id,
            f"🔎 Ищем соперника для игры на {stake} ⭐...",
        )
...
async def play_timer_handler(callback: CallbackQuery):
    ...
    async with match.lock:
        player_clicked_attr = (
            "p1_clicked_at" if user_id == match.p1_id else "p2_clicked_at"
        )

        if getattr(match, player_clicked_attr) is not None:
            return await callback.answer(
                "Вы уже сделали свой ход в этой игре.", show_alert=True
            )

        setattr(match, player_clicked_attr, time.time())
        await db.update_timer_player_click(match_id, user_id, time.time())

        opponent_clicked_attr = (
            "p2_clicked_at" if user_id == match.p1_id else "p1_clicked_at"
        )
...
async def timer_leave_active_handler(
    ...
    if details:
        ...
        await callback.answer(
            "Вы покинули игру. Ваша ставка была потеряна.", show_alert=True
        )
    else:
...
async def timer_rematch_handler(callback: CallbackQuery, bot: Bot):
    ...
    if match_id in rematch_timer_offers:
        ...
        if not await db.update_user_balance(user_id, -stake):
            await db.update_user_balance(
                offer["user_id"], stake
            )  # Возвращаем ставку тому, кто ждал
            return await callback.answer(
                "Недостаточно средств для реванша.", show_alert=True
            )
        ...
        await edit_caption_safe(
            ...
            offer["user_id"],
            offer["msg_id"],
            f"Соперник принял реванш! Играем на джекпот {old_bank} ⭐. Запуск...",
        )
        await edit_caption_safe(
            bot,
            user_id,
            callback.message.message_id,
            f"Вы приняли реванш! Играем на джекпот {old_bank} ⭐. Запуск...",
        )
        await asyncio.sleep(1)
        await start_timer_match(
            bot,
        ...
        if not await db.update_user_balance(user_id, -stake):
            return await callback.answer(
                "Недостаточно средств для предложения реванша.", show_alert=True
            )

        rematch_timer_offers[match_id] = {
            "user_id": user_id,
        ...
        await edit_caption_safe(
            ...
            user_id,
            callback.message.message_id,
            "✅ Запрос на реванш отправлен. Ожидаем ответа соперника...",
        )

        rematch_keyboard = timer_rematch_offer_keyboard(
            match_id=match_id, opponent_id=user_id, bank=old_bank
        )
        try:
...
async def timer_rematch_accept_handler(callback: CallbackQuery, bot: Bot):
    ...
    if not await db.update_user_balance(user_id, -stake):
        await db.update_user_balance(
            opponent_id, stake
        )  # Возвращаем ставку тому, кто предложил
        await callback.answer(
            "У вас недостаточно средств для реванша.", show_alert=True
        )
        return
    ...
    msg2 = await bot.send_photo(
        opponent_id, PHOTO_MAIN_MENU, caption="Соперник принял реванш! Запуск..."
    )

    await start_timer_match(
        bot, user_id, opponent_id, stake, old_bank, msg1.message_id, msg2.message_id
    )
...
async def timer_rematch_decline_handler(callback: CallbackQuery, bot: Bot):
    ...
    if match_id in rematch_timer_offers:
        ...
        await db.update_user_balance(
            opponent_id, offer.get("stake", 0)
        )  # Возвращаем ставку
        try:
            await bot.send_message(
                opponent_id, "Соперник отклонил предложение о реванше."
            )
        except Exception as e:
